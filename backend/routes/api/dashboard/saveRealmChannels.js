...
module.exports = (fastify, opts, done) => {
  fastify.post('/api/dashboard/saveRealmChannels/:id', async (req, reply) => {
    ...
    const serverData = await fastify.db.mongoFindOne(`realmLogging_${realmID}`, { guildID: serverID })
    if (!serverData) {
      await fastify.db.mongoInsertOne(`realmLogging_${realmID}`, { guildID: serverID, ...channels })
    } else {
      await fastify.db.mongoUpdateOne(`realmLogging_${realmID}`, { guildID: serverID }, { $set: channels })
    }
