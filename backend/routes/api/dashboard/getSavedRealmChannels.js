...
module.exports = (fastify, opts, done) => {
  fastify.get('/api/dashboard/getSavedRealmChannels/:id', async (req, reply) => {
    ...
    if (!serverData) return reply.status(200).send({ success: true, channels: [] }) 


    reply.send({ success: true, channels: serverData })
