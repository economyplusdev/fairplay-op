...
const banUser = require('../../../../modules/helpers/realms/banUser');
...
const calculateExpiry = require('../../../../modules/helpers/misc/calculateExpiry');
...
module.exports = (fastify, opts, done) => {
  fastify.post('/api/discordbot/banCommand', async (req, reply) => {
    const commandName = 'realm-ban';
    try {
      ...
      const expiry = calculateExpiry(time);

      const banData = { guildID: guildID, xuid: xuid, username: username, expiry: expiry, bannedBy: userID };
      fastify.db.mongoInsertOne('bannedUsers', banData);
      ...
      for (const realm of realms) {
        ...
        if (!realmObj) {
          banResults.push({
            text: `**${username}** ${await tr('could not be banned from', locale)} **Unknown Realm** ${emojis.error}`,
            success: false
          });
          continue;
        }
        ...
        const banResult = await banUser(realmID, xuid, fastify.db, guildID);
        const emoji = findEmoji(req.discordClient.guilds.cache.get(guildID), realmID);

        banResults.push({
          text: `**${username}** ${banResult.success
            ? `${await tr('has been banned from', locale)} **${realmName}** ${emoji}`
            : `${await tr('could not be banned from', locale)} **${realmName}** ${emojis.error}`
          }`,
      ...
      const embedColor = determineEmbedColor(banResults);

      const interactionEmbed = new EmbedBuilder()
        .setTitle(await tr('Ban Command', locale))
        .setThumbnail(pfp.data)
        .setColor(embedColor)
        .setDescription(`${emojis.end} ${await tr('Expires in', locale)}: <t:${Math.floor(expiry/1000)}:R>\n${formatStatusDescription(banResults)}`)

      const logEmbed = new EmbedBuilder()
        .setTitle(await tr('Ban Command', locale))
        .setColor(colors.purple)
        .setThumbnail(pfp.data)
        .setDescription(`${emojis.reply} ${await tr('Banned by:', locale)} **<@${userID}>**\n${emojis.end} Expires in: <t:${Math.floor(expiry/1000)}:R>\n\n${formatStatusDescription(banResults)}`)
